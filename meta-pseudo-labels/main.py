...
parser.add_argument(
    "--save-path", default="./checkpoint", type=str, help="save path"
)
parser.add_argument(
    "--dataset",
    default="cifar10",
...
parser.add_argument(
    "--batch-size", default=64, type=int, help="train batch size"
)
parser.add_argument(
    "--teacher-dropout",
    default=0,
...
parser.add_argument(
    "--weight-decay", default=0, type=float, help="train weight decay"
)
parser.add_argument("--ema", default=0, type=float, help="EMA decay rate")
parser.add_argument("--warmup-steps", default=0, type=int, help="warmup steps")
parser.add_argument(
    "--student-wait-steps", default=0, type=int, help="warmup steps"
)
parser.add_argument(
...
parser.add_argument(
    "--finetune-epochs", default=625, type=int, help="finetune epochs"
)
parser.add_argument(
...
parser.add_argument(
    "--amp", action="store_true", help="use 16-bit (mixed) precision"
)
parser.add_argument(
    "--world-size",
    default=-1,
...
def get_cosine_schedule_with_warmup(
    ...
    def lr_lambda(current_step):
        ...
        progress = float(
            current_step - num_warmup_steps - num_wait_steps
        ) / float(
...
def train_loop(
    ...
    for step in range(args.start_step, args.total_steps):
        if step % args.eval_step == 0:
            pbar = tqdm(
                range(args.eval_step), disable=args.local_rank not in [-1, 0]
            )
        ...
        with amp.autocast(enabled=args.amp):
            ...
            t_loss_u = torch.mean(
                -(
                    soft_pseudo_label * torch.log_softmax(t_logits_us, dim=-1)
                ).sum(dim=-1)
                * mask
            )
